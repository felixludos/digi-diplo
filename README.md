...
The primary use case here is for groups that play Diplomacy over the internet managed by a moderator.


## Discord Bot

The new (and experimental) way to use this repo is with a discord bot (recommended to be run locally). This bot provides and interface for the game directly in discord using a handful of commands (run `.help` in discord to see a full list of available commands).

The bot can be started with the `start-bot` command, and must be provided a manager (e.g. `diplo-manager`) which will keep track of the game state and actions. However, for testing purposes, you can check to make sure everything is setup correctly by starting the bot without a manager with the command: `fig start-bot`, or `fig start-bot --admins None` (for the bot to treat all users as admins).

## Scrips

The old way to running everything was with separate scripts, which are described below. However, since the current focus is on the discord bot, the scripts are currently not maintained. They will be resurrected soon (hopefully).

After the players submit their moves, the moderator can run these scripts (specifically `diplo-step` and `render`) to automatically update the game state and visualize the updated game state to send to the players. Furthermore, the moderator can use other scripts (`mapping`, `collect-pos`, `collect-fills`) to play the game with custom maps.
